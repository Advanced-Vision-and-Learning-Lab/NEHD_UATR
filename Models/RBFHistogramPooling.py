...
class HistogramLayer(nn.Module):
    def __init__(self,in_channels,kernel_size,dim=2,num_bins=4,
        ...
        if self.dim == 1:
            self.bin_centers_conv = nn.Conv1d(self.in_channels,self.num_bins*self.in_channels,1,
            ...
            self.bin_widths_conv = nn.Conv1d(self.num_bins*self.in_channels,
                                             self.num_bins*self.in_channels,1,
        ...
        # Image Data
            ...
            self.bin_centers_conv = nn.Conv2d(self.in_channels,self.num_bins*self.in_channels,1,
                                            groups=self.in_channels,bias=True)
            self.bin_centers_conv.weight.data.fill_(1)
            self.bin_centers_conv.weight.requires_grad = False
            self.bin_widths_conv = nn.Conv2d(self.num_bins*self.in_channels,
                                             self.num_bins*self.in_channels,1,
                                             groups=self.num_bins*self.in_channels,
                                             bias=False)
        ...
        # Spatial/Temporal or Volumetric Data
            ...
            self.bin_centers_conv = nn.Conv3d(self.in_channels,self.num_bins*self.in_channels,1,
                                            groups=self.in_channels,bias=True)
            self.bin_centers_conv.weight.data.fill_(1)
            self.bin_centers_conv.weight.requires_grad = False
            self.bin_widths_conv = nn.Conv3d(self.num_bins*self.in_channels,
                                             self.num_bins*self.in_channels,1,
                                             groups=self.num_bins*self.in_channels,
                                             bias=False)
    ...
    def constrain_bins(self,xx):
        ...
        if self.dim == 1:
            n,c,l = xx.size()
            xx_sum = xx.reshape(n, c//self.num_bins, self.num_bins, l).sum(2) + torch.tensor(10e-6)
            xx_sum = torch.repeat_interleave(xx_sum,self.num_bins,dim=1)
            xx = xx/xx_sum  
        
        # Image Data
        elif self.dim == 2:
            n,c,h,w = xx.size()
            xx_sum = xx.reshape(n, c//self.num_bins, self.num_bins, h, w).sum(2) + torch.tensor(10e-6)
            xx_sum = torch.repeat_interleave(xx_sum,self.num_bins,dim=1)
            xx = xx/xx_sum  
        ...
        # Spatial/Temporal or Volumetric Data
            ...
            n,c,d,h,w = xx.size()
            xx_sum = xx.reshape(n, c//self.num_bins, self.num_bins,d, h, w).sum(2) + torch.tensor(10e-6)
            xx_sum = torch.repeat_interleave(xx_sum,self.num_bins,dim=1)
            xx = xx/xx_sum   
